...
# Substances are in $3, $4, $26, $27 $28
	for(i in is){ # Columns where substances are
		...
		if(y[subs[s] ":" i]){#If an entry exists for this substance in this role i
		    if(mpy<y[subs[s] ":" i]){
			y[subs[s] ":" i]=mpy # Update only if date is older
		    }
		    # If already exists but date is larger, do nothing
		}
		else{
		    y[subs[s] ":" i]=mpy#Assign a year to a substance with role i
		}
